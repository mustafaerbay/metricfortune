...
export default auth(async function proxy(request: NextRequest) {
  ...
  if (session?.user && !session.user.emailVerified) {
    const emailVerificationRequired = ["/auth/complete-profile", "/auth/install-tracking", "/dashboard"];
  ...
  if (session?.user?.emailVerified && session.user.businessId) {
    const authPages = ["/auth/login", "/auth/signup", "/auth/verify-email", "/auth/complete-profile"];
